...
void packet_capture(u_char *args, const struct pcap_pkthdr *header, const u_char *packet){
  ...
  if (ntohs(tcp->tcp_dport) == HTTP_PORT || ntohs(tcp->tcp_sport) == HTTP_PORT) {
    u_char *data = (u_char *)(packet + sizeof(struct ethheader) + ip->iph_ihl * 4 + (tcp->H_len * 4));
    int data_len = header->len - (sizeof(struct ethheader) + ip->iph_ihl * 4 + (tcp->H_len * 4));
    if (data_len > 40) {  // 따로 조각난 패킷들은 출력하지 않기 위해 필터링
      printf("HTTP Message: ");
      for (int i = 32; i < data_len; i++) {
...
int main() {
  ...
  handle = pcap_open_live("enp0s9", BUFSIZ, 1, 1000, errbuf);
